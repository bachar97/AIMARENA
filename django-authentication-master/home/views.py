...
from nonmotiongame.models import GameMode_User, Rank
...
def leaderboards(request):
    ...
    context = {"leaderboards": scores, "Userdetails":Userdetails, "ranks":ranks}
...
def unsubscribe(request):
    ...
    x = emailnews.objects.values_list('subscription', flat=True).get(
        userid=request.session.get("userid"))
    if x:
        return render(request, "unsubscribe.html", {"x": x, "Userdetails":Userdetails})
    else:
        return redirect('game')
