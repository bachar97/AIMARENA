                ...
                if scoresforranks:
                    ...
                    return JsonResponse(
                        {"Avgacc": averageaccpermode, "Avgscore": averagescorepermode,
                         ...
                         "Accuracy": accuracy, "rankimage": (str(rankimage)), "showanimation": showanimation, "current_site":current_site})
            else:
                return JsonResponse(
                    {"Avgacc": averageaccpermode, "Avgscore": averagescorepermode,
                     "score": (float(hits) * float(accuracy)) / 10,
                     "Accuracy": accuracy, "current_site":current_site})
        else:
            print('less than 5')
            return JsonResponse(
                {"Avgacc": averageaccpermode, "Avgscore": averagescorepermode,
...
def purchase(request, pk):
    if request.method == "POST" and request.is_ajax():
        ...
        if int(productexist) == int(product_id):
            ...
            if int(userinfo) >= int(product_price):
    ...
    if request.session.get("userid") and request.session.get('RoleName') == "User":
        ...
        for i in banned:
            if i:
                ...
                return render(request, "ranked.html", {"sprites": sprites, "lastmatches": var,"Userdetails":Userdetails})
