    ...
    if post.likes.filter(id=request.session.get('userid')).exists():
        ...
        if userid != post.author_id:
            notify = Notification(post=post, sender=sender, user=post.author,
                                  text_preview=str(sender.Username) + " " + "liked your post", Notification_type=1)
            notify.save()
        liked = True
    ...
    if request.method == "POST":
        ...
        if request.is_ajax():
            ...
            if userid != post.author_id:
                ...
                notify = Notification(post=post, sender=sender, user=post.author,
                                      text_preview=str(sender.Username) + " " + "commented on your post", Notification_type=2)
                notify.save()
            return JsonResponse({"message": "success"})
        ...
        if reply_id:
            ...
            if userid != comment_qs.user.id:
                notify = Notification(post=post, sender=sender, user=post.author,
                                      text_preview=str(sender.Username) + " " + "replied to your comment", Notification_type=3)
                notify.save()
...
def search_titles(request):
    ...
    if selectedoption == "Titles":
        ...
        query = "select ur.RoleName as Role, p.id as id, p.title as title, p.body as body, p.timestamp as timestamp, u.id as uid , u.Username as Username, u.user_thumbnail as user_thumbnail FROM posts p, useregister u, Userrole ur where p.author_id = u.id and u.Role_id = ur.id and deleted=False and p.body LIKE '%%%%%s%%%%' " % (
            search_text)
...
def forumcategories(request):
    ...
    print(newposts)
    latest = Post.objects.filter(deleted=False).order_by('-timestamp')[0:3]
    print(obj)
    Userdetails = None
    if request.session.get("userid"):
        Userdetails = Userreg.objects.get(id=userid).user_thumbnail
    return render(request, "forums.html",
                  {"permission": resultsList, "obj": obj, "latest": latest, "roleslist": roleslist,
                   "Userdetails": Userdetails})
...
class forumcategroieseach(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        stuff = Post.objects.filter(topic_title__slug=self.kwargs['slug'], Featured=False, appeal_accepted=False,deleted=False,
                                    appeal_rejected=False, is_available=False, is_rejected=False).order_by('-timestamp')
        if self.request.session.get("userid"):
            Userdetails = Userreg.objects.get(id=userid).user_thumbnail
        stuffcount = Post.objects.filter(topic_title__slug=self.kwargs['slug'], deleted = False).count()
...
def typeofposts(request, posttype):
    ...
    return render(request, "Post_types.html", {"items": x, "count": count, "ans": ans, "Userdetails": Userdetails})
