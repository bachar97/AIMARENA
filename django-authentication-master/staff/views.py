...
def userprofile(request, pk):
    if request.is_ajax and request.method == 'POST':
        ...
        user_profile = Userreg.objects.filter(id=pk).values('Role_id__RoleName', 'Username', 'Firstname', 'Lastname',
                                                            ...
                                                            'User_Mode_user__Rank__Rank_image')
...
def staffsubtopicscreate(request):
    if request.method == "POST":
        ...
        subtopiccreate.save()
        return redirect("staff:staffsubtopics")
    return redirect("staff:staffsubtopics")
...
def manageproducts(request):
    ...
    if roleName == "Admin":
        ...
        return redirect('staff:baseview')
...
def editproduct(request, pk):
    if request.method == "POST":
        ...
        try:
            file_input1 = request.FILES['image1']
            file_input2 = request.FILES['image2']
        ...
        title = request.POST.get("title")
        desc = request.POST.get("desc")
        priceedit = request.POST.get("price")
        type = request.POST.get("type")
        p.objects.filter(id=product_id).update(product_name=title, product_desc=desc, product_price=priceedit,product_image= image_path1,product_spriteimage= image_path2 , product_type =type)
        return JsonResponse({"message": "success"})
